    ...
    if request.method == 'POST':
        if request.POST.get('search', False):
            ...
            if events_found == 0:
                ...
                form = SavedEventsForm(request.POST)
                if request.POST.get('save'):
                    form.name = event_list[0]
                    form.image = event_list[1]
                    form.date = event_list[2]
                    form.time = event_list[3]
                    form.venue = event_list[4]
                    form.city = event_list[5]
                    form.state = event_list[6]
                    form.address = event_list[7]
                    form.link = event_list[8]
                    form.save()
